...
const Content = () => {
  ...
  return (
    <main
      className={`${
        theme === "black"
          ? "flex flex-1 flex-col bg-primary-black transition-colors duration-700 h-full border-r border-l border-gray-700"
          : "flex flex-1 flex-col h-full border-r border-l border-gray-300 bg-white transition-colors duration-700"
      }`}
    >
      {/* Header */}
      <header
        className={`${
          theme === "black"
            ? "bg-primary-black text-white flex sticky top-0 z-50 flex-row h-16 border-b border-gray-700 justify-between transition-colors duration-700 px-4 py-4   "
            : "bg-white text-black flex sticky top-0 z-50 flex-row h-16 border-b border-gray-300 justify-between transition-colors duration-700 px-4 py-4 "
        }`}
      >
      ...
      {/* Add Tweet Area */}
      <div className="flex flex-row h-44 md:w-full sm:w-20 my-4 pl-4">
        <img
          src={profileImage}
          alt="Profile Image"
          className="w-12 h-12 rounded-full hover:opacity-80 transition-opacity duration-300"
        />
        <div className="pl-3 w-full flex flex-col">
          <div className="h-full ml-2 flex flex-col">
            <textarea
              ...
              className={`${
                theme === "black"
                  ? "w-full pb-28 bg-transparent  text-white text-xl focus:outline-none break-words resize-none overflow-y-hidden border-b transition-colors duration-700 border-gray-700 mb-5"
                  : "w-full pb-28 bg-transparent  text-black text-xl focus:outline-none break-words resize-none overflow-y-hidden border-b transition-colors duration-700 border-gray-300 mb-5"
              }`}
            ...
            <div className="flex justify-between">
              <div className="flex flex-row items-center">
                {loading && <LoadingIcon className="mb-10 mx-5" />}
